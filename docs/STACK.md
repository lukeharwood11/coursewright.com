...
| **Realtime (P1 discussions)** | **Supabase Realtime** | Postgres changes on discussion tables while the SPA is open. RLS still applies. Not email and not typing indicators. Device notifications are the Activity push row, not Realtime |
| **Activity push** | **Web Push** (VAPID) | Installed PWA only. `send-activity-push` reads unread Activity rows. Public key from `web-push-public-key`. Secrets are **HN-018** |
...
16. **Realtime is opt-in per product surface** — **P1 discussions** subscribe to Postgres changes through the existing Supabase browser client while the SPA is open. RLS still gates payloads. Do not add a second websocket stack. Activity device notifications are Web Push from the installed PWA, not Realtime.
