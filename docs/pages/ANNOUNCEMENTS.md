...
Owners, admins, and instructors in **Teacher view** use the staff list (compose). Students (and staff **Student view**; linked parents inherit) use the same URL for a student list of **current** notices — no compose.

## Purpose

Staff: list of one-way announcements in the organization, grouped by whether students see them now.  
Students: all current notices that apply to them, with unread vs seen state.
...
### Students (and staff Student view; linked parents inherit)
...
- No **New announcement**. Staff Student view without linked students sees an empty list (preview).

Empty: **There are no announcements at this time.** **New announcement** still shows for staff when compose is allowed.
...
- Audience / targets; which student when multi-student
