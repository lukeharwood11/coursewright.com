...
| Grain | PDF content |
|-------|-------------|
| **Material** | One material by kind — **page:** blocks in order (quizzes print on the whole page: **parents** see questions only; **staff** see the answer key from block data); **link:** title + URL/QR; **file:** if the attached file is already a PDF/image, prefer previewing **that file**; otherwise a cover sheet (title, description). Video blocks → title + URL/QR, not a player frame. No quiz-block-only print in P0. |
