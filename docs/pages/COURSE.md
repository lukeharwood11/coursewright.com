...
Instructors (builder). Admins as needed. Students: read-focused variant of the same tree (simpler chrome).
...
- **Events:** list of events for this course (when, location). **Add event** starts a course event with this course selected. A course event stays on this one course. Opening a row goes to [EVENT](./EVENT.md). Students see events they can already open.
- **Start a discussion** (**P1**): same course as the audience (`…/discussions/new?audience=course&courseId=`). Students on the student variant can start one when they are enrolled (active + published). On small screens this action lives under **More**.
...
- Staff **Student view** uses this variant (unpublished materials omitted; no edit)
