...
- **Week** view: Sunday–Saturday columns using the same week calendar as [ORG_HOME](./ORG_HOME.md). Each day, per visible course: day plan text (if any), then a divider, then that day’s lesson-plan materials **and** assigned/due materials (deduped). Week notes sit as a colored bar per course above the grid.
