...
- Main body is the **current Sunday–Saturday week** (same chips, colors, and week notes as [CALENDAR](./CALENDAR.md)): **one card per class on a day** that wraps (about two or three across). Empty days are omitted. Each card keeps that class’s plan text and its linked materials together (after a divider), so multiple classes on the same day stay distinct. Assigned = outline / due = filled, course colors.
...
- Week calendar: **cards** for each class on a day that has a plan or assigned/due work (empty days omitted; cards wrap instead of a squeezed seven-column row). Each card: course + day plan text; divider; that class’s lesson-plan materials and assigned/due materials (title; **Assigned** outline / **Due** filled; course color)
...
- Two columns on large screens: **week cards** | **Focus** (Important now + Coming up). Stacks on small screens (calendar first). Class-day cards wrap so two or three fit across instead of seven skinny columns.
