...
- Main body is the **current Sunday–Saturday week** (same chips, colors, and week notes as [CALENDAR](./CALENDAR.md)): days with work are **cards** that wrap (about two or three across). Empty days are omitted. One card per day: **events** for the active students (title chip), then each class’s plan text and its linked materials stay together inside that card (after a divider), assigned = outline / due = filled, course colors. An event-only day still shows.
