...
- **page:** ordered **blocks** rendered from the Lexical document (rich text + video embeds)
- **link:** URL (+ title)
...
- **page** — Lexical editor: headings, lists, links, **video** URL embeds; printable block layout
...
- **page:** [Lexical](https://lexical.dev/) WYSIWYG (headings, lists, links, video URL embeds). A version is stored only when the instructor saves and something changed — not per keystroke.
