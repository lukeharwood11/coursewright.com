...
- **Grade questions automatically and show the score right away** on: the submitter sees a score like **Score 4.5/5 (90%)** when every question received points. Off: the entry is saved with no score. Later edits to the key do not change a saved score.
- Each question has possible points, default **1**. The edit page shows the total at the top. Fractions such as **4.5** are allowed.
