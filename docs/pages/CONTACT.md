# CONTACT

**URL:** `/contact`  
**URL map:** [URLS.md](../URLS.md)

## Audience

Anyone (public) who needs to reach Course Wright.

## Purpose

Publish the public email addresses for general/partnership, support, and legal/privacy inquiries.

## Behavior

- Public marketing page; no form — mailto links only.
- Addresses live in marketing `model/contactEmails.ts` and are reused on [PRICING](./PRICING.md) (`hi@`) and [PRIVACY](./PRIVACY.md) (`legal@` / support / hi as appropriate).

## Data shown

- Two inboxes with short purpose copy:
  - `hi@coursewright.com` — general & partnerships (incl. pilot interest)
  - `support@coursewright.com` — product support
- Do **not** list `legal@` here — that address is only advertised on [PRIVACY](./PRIVACY.md)

## Contents

- Heading + short intro
- Email directory cards with mailto links
- Back to home
- Shared marketing chrome

## Primary actions

- Email the right inbox
- Go home

## Links to

- [HOME](./HOME.md)
- mailto: addresses (external)
- via marketing footer: [HOME](./HOME.md), [ABOUT](./ABOUT.md), [PRICING](./PRICING.md), [LOGIN](./LOGIN.md), [SIGNUP](./SIGNUP.md), [CONTACT](./CONTACT.md), [PRIVACY](./PRIVACY.md), [COOKIES](./COOKIES.md), [CONSTRUCTION](./CONSTRUCTION.md) (`/terms`)

## Notes

Do not invent additional channels (phone, chat) unless product docs say so.
