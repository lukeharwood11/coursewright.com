...
|---------|-------------|--------|-------|
| **Marketing site** | Public home, about, pricing, contact, privacy, cookies, **help docs** | shipped | Home/about stay general (no named third-party tools). Pricing is **invite-only pilot** — no plan options; pilot interest → `hi@coursewright.com`. Contact emails live. Privacy + cookie policies live (PostHog + standard disclosures). Help at `/docs` (Getting started + nested topics; header/footer **Help**). Terms still a construction placeholder |
