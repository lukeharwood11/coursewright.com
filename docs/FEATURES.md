...
| **Admins** | Same org management as owners (name, permalink, grade scheme, school days, profile, staff) except **billing** and **branding** |
...
| **Score** | Multiple choice is one point when the selected choices match the correct set exactly. A number is one point when it is the same value (`3.5`, `3.50`, and `7/2` match). Matching is one point when every pair is exact. Short answer and long answer are stored for the teacher and are not part of the automatic score. A long answer prints the number of blank lines the teacher chose (1–20). **Grade questions automatically and show the score right away** (default off) freezes the auto score on that entry. Families see **Score 4/5 (80%)** on the outline and entry only when every answer is graded — short/long (and any other unscored item) stay **Yet to be graded** until the teacher marks **Correct** or **Incorrect**. Until then a submitted entry shows a submitted check, not the score |
...
| **Not this slice** | Manual points toward the score total, a max-attempt count, Activity, a gradebook, quizzes on course templates, and changes to page quiz blocks |
