...
|---------|-------------|--------|-------|
| **Marketing site** | Public home, about, pricing, contact, privacy, terms, cookies, **help docs** | shipped | Home hero is the pre-ladder wording (“Courses, done wright.”). Public `/pricing` is a display-only ladder — Family free (2 students, 1 collaborator, 1 GB), Family Pro $10/mo · $100/yr, Microschool $80/mo · $800/yr, School $150/mo · $1,500/yr — yearly prices are 10× monthly and display lower whole-dollar effective monthly rates, default monthly, **Details** anchors to visible feature explanations; **Contact us** on paid cards only, no checkout. Billing remains the P1 Stripe stub. Contact emails live. Privacy, terms, and cookie policies live (PostHog + standard disclosures). Help at `/docs` (Getting started + nested topics; header/footer **Help**). SEO: build-time `sitemap.xml` / `robots.txt` (index `coursewright.com` only; disallow `/my/` + `/invite/`), OG/meta + JSON-LD in `index.html`, `site.webmanifest` (PWA pt 1: standalone + `/my` start URL), `llms.txt`, `.well-known/security.txt` |
| **Authentication** | Sign up, sign in | shipped | **Email (password or magic link) + Google** via **Supabase Auth**. Signup is Google or **email + password** (first and last name required) and **signs the person in** (no extra login step). Login keeps password + magic-link. Session gates live |
...
| **Course builder** | Create and organize **courses** within an org (no templates in P0) | shipped | Create, course home, units, materials (page/link/file), print/share chrome; collapsible course outline (units + materials tree). **Linked resources** — shortcuts at the bottom of course content to org Resource folders or items (owners / admins / course instructors manage links) |
...
| **Advanced search** | Native, easy, **cross-facet** search — “where do I have this resource?” | in progress | Org chrome overlay for all members (no `/search` route): Postgres FTS on `search_vector` for courses + materials; staff pages by title. Findability only (RLS). Deferred: facets, files, units, people, `ts_rank`, dedicated index |
...
| **Print materials** | One-tap print of a material, a unit, or this week's work | shipped | [PRINT](./pages/PRINT.md): `@react-pdf/renderer` + in-app preview, Download / Print. Page materials never print page-quiz answer keys. **Course quizzes** on print-this-week, print unit, and quiz print: worksheet / answer key / both when allowed (`qid`, `quizKey`). This-week: options panel (lesson plans, linked materials, important now, assigned/due, quizzes; pack + page-break controls) with URL-persisted `omit` / `break` / `pack` / `studentBreaks` / `qid`; home still one-taps with everything on. Whole-course print out of P0 |
| **Lesson materials & planning** | Unified storage for course content, files, and plans | shipped | Course builder authoring on courses |
| **Content versioning** | Versions of course content; who changed what; revert dangerous actions | shipped | Material edit **Version history** dialog: browse snapshots, preview, restore (who · when on each version). File blob revert on file materials. Page/placement edits version only on Save when something changed. **Resources** edit uses the same dialog for document / link / file items (`org_resource_versions`) |
...
| **Student home (this week)** | This week’s **calendar** (lesson plans + assigned/due chips), **Focus** (Important now + Coming up), **announcements**, student tags, **Print this week** | shipped | Student home; staff **Preview / Parent / Student** modes use the same chrome. Linked parents inherit this presentation. **Previous week** / **Next week** / **This week** on org home (`?week=` Sunday ISO). Other weeks show calendar (announcements as-of that Sunday; Coming up still uses today); **Important now** only on the real current week. Current **announcements** (unread first, notification icon until opened; check icon once seen) sit above the week cards. Sidebar **Announcements** lists the same current notices and shows a red unread count. Main body is the selected Sunday–Saturday week as **wrapping day cards** (empty days omitted). Focus rail (right on desktop, below on small screens) holds **Important now** and **Coming up**. Student tags still filter who is shown. **Print this week** prints the visible week (published lesson plans first, then important now + dated materials, one student at a time) |
...
| **Staff view modes** | Owners, admins, and instructors switch most org pages to Preview or family presentation | shipped | Header **Teacher** / **Preview** / optional **Parent** / **Student**. **Preview** = student chrome as if enrolled in published courses you teach (not your kids). **Parent** (if `is_parent`) = real linked children. **Student** (if `is_student`) = own linked profile. Mobile uses a Select when 3+ options. Hidden for parent-role / student-role users (they always see student chrome). Observers have no toggle |
...
**This slice (org chrome):** Overlay search uses generated `search_vector` GIN indexes via PostgREST `fts(english)` (`to_tsquery`, not `plainto_tsquery`) for **courses** and **materials**, so prefixes like `frac` match Fractions. Staff **pages** (Home / Courses / Students / Settings) match by title in the client. All org members see the top-bar search; no `/search` route; no Algolia/Elastic/side index. Hits are findability only — they do not grant access; enrollment / `parent_student_links` stay the gate.

**Deferred:** facets, files, units, roster people / families, page/block body text, `ts_rank`, dedicated `/search` route.
...
**Staff view modes (P0):** Owners, admins, and instructors get a **Teacher** / **Preview** control in org chrome (not parent-role or student-role users). **Preview** uses student chrome and a synthetic this-week for **published courses they teach** (`course_instructors`) — not linked children’s enrollments. If the staff member is also a parent (`is_parent`), a **Parent** tab shows their real family home (`parent_student_links`). If they are also a linked student (`is_student`), a **Student** tab shows that own profile. Both additive tabs can appear. Staff-only destinations (roster, settings, material edit) return to org home while any non-Teacher mode is on. **Courses** list in Preview is taught + published only. Default is Teacher. Print packets omit the answer key in Preview / Parent / Student modes. On mobile, 3+ modes use a Select instead of cramped tabs.
...
A **Discussion** is a **two-way** thread (title + posts) aimed at **one course**, **one class**, or the **organization**. Staff can set **family visibility** to parents only, students only, or both so student accounts can be excluded. It is **not** an announcement (those stay one-way, no replies) and **not** a lesson plan.
...
Staff **Preview / Parent / Student** modes use the student rules (create only if they have linked students in Parent/Student). Teacher view uses the staff rules. Preview without taught published courses is an empty preview — no compose.
...
| **Answer key** | Teacher view always. **Share answer key with parents** (default off) shows it to parents whenever the quiz is published. A student login (account email matches that student’s `student_email`) never sees it. Staff **Preview / Parent / Student** modes follow the parent rule |
...
| Org profile (about, address, website, contact email, phone) | **Decided** | Optional; org settings. Shown on org profile page when set. Not a public about page |
...
| P1 discussion = two-way thread for one course, one class, or organization | **Decided (in design)** | Organization threads (staff start); **family_audience** parents / students / both; families create for a course their child is enrolled in or a class their child is in; everyone on the thread can post; flat timeline + **Quote** in message body; plain composer + **T** → Lexical; author or staff marks answered; attachments = file / material / URL; Realtime while the SPA is open; not email; not an announcement |
