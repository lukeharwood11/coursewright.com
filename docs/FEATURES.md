...
| **Calendar** | Month and week view of assigned/due work and lesson plans, color-coded by course | shipped | Sidebar **Calendar** for staff and parents. Assigned = outline chip; due = filled chip. Course colors from a small palette (`courses.color_key`) with a filterable legend. Week view shows lesson-plan text in seven columns; This week uses wrapping class-day cards and hides empty days |
...
| **(A) This week** | P0 | Current Sunday–Saturday week as **wrapping class-day cards** (empty days omitted): one card per class on a day, plan text + that class’s materials after a divider, assigned = outline / due = filled. **Focus** rail: Important now + Coming up |
