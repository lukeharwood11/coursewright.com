...
| **Customizations** | **Owners only.** Turn optional product surfaces on or off for the organization: Discussions, Announcements, Resources, Lesson plans, Events, Calendar view. Off hides nav, routes, and compose entry points; existing data is kept. Admins see the settings read-only. Defaults: all on |
...
| **Org feature customizations** | Owners turn optional surfaces on or off: Discussions, Announcements, Resources, Lesson plans, Events, Calendar view | shipped | Org settings **Customizations** tab. `organization_features` (owner writes; members read). Missing row = all on. Off hides sidebar items, routes, and compose buttons; does not delete data |
