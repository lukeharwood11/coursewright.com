...
|---------|-------------|--------|-------|
| **Marketing site** | Public home, about, pricing, contact, privacy, terms, cookies, **help docs** | shipped | Home/about stay general (no named third-party tools). Pricing is **Free (for now)** — no plan options; partnership interest → `hi@coursewright.com`. Contact emails live. Privacy, terms, and cookie policies live (PostHog + standard disclosures). Help at `/docs` (Getting started + nested topics; header/footer **Help**) |
