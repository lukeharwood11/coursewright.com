...
| **Advanced search** | Native, easy, **cross-facet** search — “where do I have this resource?” | in progress | Staff chrome overlay (no `/search` route): Postgres FTS on `search_vector` for courses, units, materials, files (filename → material). Type facet: all / page / course / unit / material / file. Still open: `ts_rank`, page-body text, more facets, people / families, parent search |
...
**Shipped (staff chrome):** Overlay search uses generated `search_vector` GIN indexes (prefix `to_tsquery` via PostgREST). Indexed fields today: course title/description/subject/location; unit title; material title/description; file filename. File results open the material that uses the file (file material or in-page attachment). Type is the only facet. Ranking is type + title match in the client — not Postgres `ts_rank`.

**Still open:** page/block body text, extra facets (kind, grade, dates, important now), `ts_rank`, roster people / families, parent search, dedicated `/search` route.
