...
|---------|-------------|--------|-------|
| **Marketing site** | Public home, about, pricing, contact, privacy, terms, cookies, **help docs** | shipped | Home/about stay general (no named third-party tools). Pricing is **Free (for now)** — no plan options; partnership interest → `hi@coursewright.com`. Contact emails live. Privacy, terms, and cookie policies live (PostHog + standard disclosures). Help at `/docs` (Getting started + nested topics; header/footer **Help**). SEO: build-time `sitemap.xml` / `robots.txt` (index `coursewright.com` only; disallow `/my/` + `/invite/`), OG/meta + JSON-LD in `index.html`, `site.webmanifest` (PWA pt 1: standalone + `/my` start URL), `llms.txt`, `.well-known/security.txt`. Legal routes also emit static HTML at build (`privacy` / `terms` / `cookies`) so non-JS verifiers see policy text |
...
| **Audio & video files** | Video as a **block** on a material page; uploaded audio via **file** materials / in-page file attachments | shipped | Video **URL embed** in page blocks (upload vs URL still **TBD**). Uploaded audio uses a shared custom in-app player (play/pause, scrub, time, 1×/1.5×) on file materials and in-page files; uploaded video still uses native `<video>`. Instructors can **record a microphone clip** (under 5 minutes) when adding or replacing a file material |
...
| **Notifications** | In-app **Activity** list of events that need a person’s attention. Discussion posts notify the **course instructors** or **class leads**. **@mentions** notify that person if they can see the thread. Staff starting a discussion can opt in to **Notify everyone** on the thread. Clicking a row marks it read and opens the activity | shipped | Stored `notifications` rows (ack = `read_at`). Rows show a type icon + headline (e.g. **New discussion: … in …** / **Mentioned in …**), not a read-receipt checkmark. Header **bell** (right of the avatar) with unread badge; dropdown previews the three newest unread or **You're all caught up!**, plus **View all activity**. Not email or push in this slice. Announcement opt-in email stays P0 (**Send notification**). Discussion thread unread badge is separate |
...
| **Record audio snippet** | When adding (or replacing) a **file** material, record a clip under 5 minutes with the device microphone | shipped | Still a `file` material — not a new kind. Browser `MediaRecorder`; upload via existing Storage path |
...
- As an **instructor**, I want to **record a short audio clip when I add a file** so that **I don’t need a separate recorder app**.
...
| Rule | Detail |
|------|--------|
| **Who is notified of a discussion post** | **Course instructors** for a course thread; **class leads** for a class thread. **@mentions** also notify that person if they can see the thread. Never the author. |
