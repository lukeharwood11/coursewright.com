...
| **Org white labelling** | Owners upload a small icon and set one accent color for the org sidebar and header | shipped | Account org list shows the icon. Page content, emails, and print stay Course Wright. `organization_branding` + public `org-brand` bucket. Owners only write. The icon file and `organization_icons` path are public so a future invite can show the picture without a membership. Accent stays member-only |
