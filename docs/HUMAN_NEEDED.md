...
1. Set secrets on both tiers (project refs come from Terraform state — testing branch and production main). Production is included only with `--yes`:
   `./scripts/setup-activity-push.sh --yes`
   Testing only: `./scripts/setup-activity-push.sh testing`. The script generates a VAPID pair and webhook per tier, sets `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, and `ACTIVITY_PUSH_WEBHOOK_SECRET`, and upserts Vault `activity_push_webhook_secret` plus `activity_push_function_url` (`https://<ref>.supabase.co/functions/v1/send-activity-push`). The webhook value matches `ACTIVITY_PUSH_WEBHOOK_SECRET`. Re-running replaces the key pair, so installed apps must turn notifications on again.
2. Deploy so the migration and functions are on that project: `./scripts/deploy-supabase.sh testing` (and `./scripts/deploy-supabase.sh production` when that tier should send).
3. On the matching site, install Course Wright (home screen or installed window), open it, choose **Turn on notifications**, then cause an Activity row (a discussion post or **Send notification** on an announcement). The device should show the same headline as Activity.
