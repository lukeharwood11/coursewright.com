...
**Who can edit** a quiz and its questions, choices, answer keys, and matching rows: `can_manage_course`. That is an active org **owner** or **admin** (no `course_instructors` row required) or an **assigned** course instructor (a `course_instructors` row plus an active owner/admin/instructor membership). An org instructor with no assignment cannot add or change questions. Write checks are security-definer helpers so they do not re-read `quizzes` under `quizzes_select`.
