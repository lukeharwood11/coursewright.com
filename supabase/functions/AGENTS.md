...
- One folder per use case, e.g. `create-course-from-course/` (**P0**), `send-organization-invite/` (Resend), `send-announcement-notification/` (Resend), `send-report-card/` (Resend event `report-card`, **HN-019**), `send-activity-push/` (Web Push), `web-push-public-key/` (VAPID public key), `create-course-from-template/` (**P1**)
...
- **P0 candidates:** create-course-from-course (copy units/materials/blocks/quizzes), send-organization-invite (Resend), send-announcement-notification (Resend), invite claim, revert cascades.
