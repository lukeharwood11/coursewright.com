-- Store matching picks so the take form can refill after submit.

alter table public.quiz_attempt_answers
  add column if not exists match_pairs jsonb not null default '[]'::jsonb;

comment on column public.quiz_attempt_answers.match_pairs is
  'Matching picks as [{leftId, rightId}, …], frozen at submit.';
...
comment on function public.submit_quiz_attempt(bigint, bigint, jsonb) is
  'Record one quiz entry during the accepting window. When autograde is on, freezes the score and per-question correctness. Matching picks are stored for review.';
