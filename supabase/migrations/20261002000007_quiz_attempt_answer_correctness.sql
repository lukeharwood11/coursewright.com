-- Freeze per-question correctness on each attempt when autograde is on.
-- Null means not scored yet (short/long answer, or no key for that question).

alter table public.quiz_attempt_answers
  add column if not exists is_correct boolean;

comment on column public.quiz_attempt_answers.is_correct is
  'Frozen at submit when autograde is on. True/false for scored kinds; null when not scored or autograde was off.';
    ...
    if question.kind = 'multiple_choice' then
      ...
      if autograded then
        ...
        if private.quiz_number_value(key_text) is not null then
          v_score_total := v_score_total + 1;
          v_is_correct := (
            private.quiz_number_value(short_text) = private.quiz_number_value(key_text)
          );
          if v_is_correct then
            v_score := v_score + 1;
          end if;
...
comment on function public.submit_quiz_attempt(bigint, bigint, jsonb) is
  'Record one quiz entry during the accepting window. When autograde is on, freezes the score and per-question correctness.';
