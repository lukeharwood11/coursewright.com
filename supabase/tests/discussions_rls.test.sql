...
select cl.id, sp.id
from classes cl
join org_profiles sp on sp.organization_id = cl.organization_id
...
join org_profiles parent
 ...
 and parent.user_id = 'd2222222-2222-2222-2222-222222222222'
where kid.name = 'Kid Discuss';
...
-- The parent cannot see these rows, so an INSERT … SELECT would insert nothing.
-- Use ids captured as the table owner so the insert is attempted and RLS runs.
reset role;
select set_config(
  'test.discussion_org_id',
  (select id::text from organizations where name = 'Discussion RLS Co-op'),
  true
);
select set_config(
  'test.unpublished_course_id',
  (
    select c.id::text
    from courses c
...
select throws_ok(
  $$
    ...
    values (
      current_setting('test.discussion_org_id')::bigint,
      'course',
      current_setting('test.unpublished_course_id')::bigint,
      null,
      'Sneaky unpublished',
      'd2222222-2222-2222-2222-222222222222'
    )
...
select throws_ok(
  $$
    ...
    values (
      current_setting('test.discussion_org_id')::bigint,
      'course',
      current_setting('test.other_course_id')::bigint,
      null,
      'Sneaky other course',
      'd2222222-2222-2222-2222-222222222222'
    )
...
select throws_ok(
  $$
    ...
    values (
      ...
      'class',
      null,
      current_setting('test.friday_class_id')::bigint,
      'Sneaky other class',
      'd2222222-2222-2222-2222-222222222222'
    )
  $$,
...
select throws_ok(
  $$
    ...
    values (
      current_setting('test.discussion_org_id')::bigint,
      'course',
      current_setting('test.published_course_id')::bigint,
      null,
      'Outsider thread',
      'd3333333-3333-3333-3333-333333333333'
    )
