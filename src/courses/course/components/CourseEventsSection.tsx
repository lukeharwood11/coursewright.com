...
export function CourseEventsSection({
  orgSlug,
  courseId,
  events,
  canEdit,
    ...
    <section className="mt-8">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {canEdit ? (
          <ButtonLink
            variant="secondary"
            to={newEventPath(orgSlug, { audience: "course", courseId })}
          >
      ...
      {events.length === 0 ? (
        <p className="mt-2 text-[13.5px] text-[var(--ink-soft)]">
          {canEdit ? "No events for this course yet." : "No events from this course right now."}
        </p>
      ) : (
