...
- **Allow more than one attempt** (default off). **Grade multiple-choice questions automatically and show the score right away** (default off). **Share answer key with parents** (default off).
- Answer keys live in `quiz_answer_keys`. A student login (account email matches that student’s `student_email`) never sees the key. Staff Teacher view always does.
...
- Course-from-course copies quizzes, questions, choices, and keys. It does not copy attempts.
